...
def find_chilean_locations_in_text(text):
    ...
    while current_index>=0:
        ...
        if not could_be_country and not could_be_region and not could_be_chilean_city:
            current_index = pulled_index
            continue
...
def setup_chilean_cities_cache(cursor):
    select = 'SELECT * FROM cities where country="cl";'
    cursor.execute(select)
    candidate_rows = cursor.fetchall()
...
def is_city(cursor, text, text_starting_index, previous_result):
    ...
    while pulled_word_count < geodict_config.word_max:
        ...
        if current_word == '':
            ...
            select = 'SELECT * FROM cities WHERE last_word=%s AND country="cl"'
            values = (pulled_word, )
